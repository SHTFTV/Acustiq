...
import {
  SystemAuthorityPage,
  SystemsAuthorityIndex,
  authoritySystemSlugs,
...
import {
  ...
  LegacyIndexPage,
  legacyGuideSlugs,
} from "./LegacyCeilingPages.tsx";
import "./index.css";
import "./legacy-ceilings.css";
...
export function Root({ pathOverride }: { pathOverride?: string } = {}) {
  ...
  if (legacySlug && legacyGuideSlugs.includes(legacySlug))
    return <LegacyGuidePage slug={legacySlug} />;
  const systemMatch = path.match(/^\/systems\/([^/]+)$/);
  const systemSlug = systemMatch?.[1];
  if (systemSlug && authoritySystemSlugs.includes(systemSlug))
    return <SystemAuthorityPage slug={systemSlug} />;
  const match = path.match(/^\/installers\/([^/]+)$/);
  const slug = match?.[1];
  if (slug && regionalCities.some((city) => city[0] === slug))
    return <RegionalInstallerPage slug={slug} />;
  return (
    <>
      <App ssrPath={pathOverride} />
      {!pathOverride && <Enhancements />}
    </>
  );
}
